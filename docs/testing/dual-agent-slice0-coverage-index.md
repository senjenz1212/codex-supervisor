...
| P0 credential boundary | `test_p0_credential_boundary_checks_direct_route_and_redaction`, `test_p0_fails_when_successful_model_call_hits_proxy_instead_of_anthropic`, `test_p0_fails_without_required_spawn_env_evidence`, `test_p0_fails_when_anthropic_proxy_environment_is_present` | Covered |
...
| Task-complexity workflow routing | `test_select_workflow_route_uses_explicit_trivial_depth`, `test_select_workflow_route_infers_modes_and_aliases`, `test_run_dual_agent_workflow_records_trivial_route_and_skips_planning_gates`, `test_run_dual_agent_workflow_small_route_requires_only_prd_skill_receipts`, `test_run_dual_agent_workflow_happy_path_owns_full_lifecycle`, and `test_read_gate_transcript_includes_skill_receipt_validation` cover explicit trivial/small/large/vague routes, alias/inference behavior, route ledger events, and transcript visibility. | Covered |
...
| Failure taxonomy and trace envelope | `test_failure_taxonomy_classifies_receipt_and_skill_gaps`, `test_blocking_probe_failure_includes_planning_and_claim_probes_but_not_p4`, `test_failure_taxonomy_triggers_all_mast_modes_through_payload_or_sequence_rules`, `test_mast_coverage_matrix_reports_every_mode_and_sequence_sources`, `test_trace_envelope_stamps_dual_agent_payloads_without_wrapping_original_shape`, `test_blocking_red_probe_beats_accepted_completion`, and artifact-export tests prove dual-agent events carry deterministic MAST-inspired failure categories, all 14 MAST modes have deterministic probe surfaces, and exports write `mast-coverage.md` plus `replay/mast-coverage.json` for operator inspection. | Covered |
| Codex rich reviewer packet | `test_codex_confidence_report_and_review_packet_explain_decision`, `test_codex_review_packet_links_claim_failures_to_receipt_replay`, `test_run_dual_agent_workflow_review_packet_blocker_forces_next_round`, `test_agent_mailbox_message_carries_trace_fields`, workflow driver round tests, and artifact export tests prove Codex emits a structured per-gate review packet with requirements, severity-ranked findings, receipt replay evidence, round policy, confidence criteria, and would-change-if criteria. | Covered |
| Multi-trial probe cohorts and SP-N proposals | `test_probe_cohort_marks_repeated_expected_blocks_stable`, `test_probe_cohort_marks_single_outlier_as_drift_one`, `test_probe_cohort_marks_no_majority_as_flipping`, `test_live_failure_mode_cohort_wrapper_writes_artifacts_with_trial_identity`, and `test_stability_proposals_*` cover STABLE/DRIFT-1/FLIPPING aggregation, unique trial task/run IDs, artifact outputs, deterministic review-only SP-N proposals, and evidence refs. | Covered at deterministic wrapper boundary |
| Forward-only schema migrations | `test_forward_migration_adds_resume_requested_at_to_old_actions_table`, `test_forward_migrations_are_idempotent`, `test_forward_migration_name_mismatch_fails_closed`, `test_forward_migration_unknown_future_version_fails_closed`, and `test_state_constructor_applies_forward_migration_to_old_db` cover migration application, idempotence, mismatch rejection, future-version fail-closed behavior, and the real `State(...)` constructor path. | Covered |
| Replay schema version drift | `test_replay_schema_versions_accept_current_manifest_versions`, `test_replay_schema_versions_accept_real_exported_manifest_keys`, `test_replay_schema_versions_reject_unapplied_known_migration`, `test_replay_schema_versions_reject_unknown_future_versions`, and `test_export_dual_agent_run_artifacts_writes_replay_manifest_with_handoff_content` cover current exported manifests, fail-closed rejection of known-but-unapplied migrations, future-version rejection, and artifact-export compatibility checks. | Covered |
| Cursor tri-agent reviewer policy | `test_build_cursor_prompt_is_review_only_and_uses_typed_outcome_contract`, `test_cursor_accepts_requires_green_probe_and_accept_decision`, `test_select_cursor_model_defaults_to_catalog_auto_route`, `test_cursor_review_gate_profiles_are_policy_not_prompt`, `test_run_dual_agent_workflow_runs_cursor_review_by_default`, `test_run_dual_agent_workflow_rigorous_cursor_profile_reviews_quality_gates`, `test_run_dual_agent_workflow_vague_route_forces_cursor_review`, and `test_run_dual_agent_workflow_with_cursor_review_blocks_on_cursor_rejection` cover default final-gate review, rigorous quality-gate review, vague planning review, and non-accept blocking. | Covered at fixture boundary |
...
failure taxonomy. The workflow now records `dual_agent_workflow_route` so task
complexity can select a reduced gate sequence for trivial/small work while the
...
- Repeated probe evidence should use `scripts/probe_live_failure_mode_cohort.py`
  to produce a cohort summary, STABLE/DRIFT-1/FLIPPING classification, and
  review-only SP-N stability proposals. Replay version drift is checked through
  `supervisor.replay_versions` and documented in
  `docs/testing/dual-agent-version-drift-replay.md`.

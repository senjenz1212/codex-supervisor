# Agentic Executor Wiring Inline TDD Plan
...
`run_dual_agent_workflow` is the MCP/CLI workflow tool boundary exposed by `codex_supervisor_mcp`; it is not expected to appear as a standalone `def run_dual_agent_workflow` symbol in `supervisor/*.py`.
...
Maps to: ISS-1, P1, P3
RED: Run an allowed agentic workflow where the roster planner returns no machine-readable worker receipts; assert the producer attempt is recorded and synthesis still proceeds instead of treating missing receipts as either success or a hard block.
GREEN: Keep `allowed` as a provenance-improving, non-blocking mode: the supervisor records blocked/no-receipt production, P13 reports accepted policy with no blocking findings, and `/lead` synthesis continues.
...
Maps to: ISS-2, ISS-5, P6
RED: Feed a lead-planned roster whose worker asks for timeout and budget above policy caps; assert the fan-out runner is not called.
GREEN: Reject over-timeout and over-budget roster items before subprocess launch, recording `worker_timeout_out_of_bounds` and `worker_budget_out_of_bounds` blocking findings without clamping or launching.
...
Maps to: ISS-5, P6
RED: Simulate a worker subprocess timeout and assert the resulting receipt can be replayed from durable stdout, stderr, transcript, and output refs but does not let required policy advance as successful runtime-native evidence.
GREEN: Persist failed-worker refs under the supervisor-owned worker directory, return `status=timeout` / `decision=revise`, and let existing P13 policy block with `subagent_status_not_passing`.
...
RED: Add the allowed-policy no-receipt workflow test.
GREEN: Record the producer attempt and continue synthesis for `agentic_lead_policy=allowed` while keeping `required` fail-closed.
...
RED: Add named over-budget/over-timeout roster rejection and timed-out worker failed-receipt tests.
GREEN: Reject over-limit specs before launch, call orphan cleanup on timeout/dead-worker paths, and preserve failed-worker durable refs while P13 blocks timeout receipts as non-passing evidence.

...
### Finding 10: P6 forbidden outcomes need named fail-closed tests

status: resolved

question: Are over-limit roster specs and timed-out worker receipts only implied by broad cleanup tests?

resolution: The TDD plan now names `test_agentic_roster_validation_rejects_over_budget_or_timeout_before_launch` for pre-launch budget/timeout cap rejection and `test_agentic_timeout_receipt_blocks_runtime_native_and_preserves_failed_refs` for timeout receipts that keep durable refs but are blocked by P13 as non-passing evidence.

### Finding 11: Allowed policy must be pinned separately from required policy

status: resolved

question: Does the plan prove `agentic_lead_policy=allowed` accelerates opportunistically without becoming either silent success or required-policy hard block?

resolution: The TDD plan now names `test_run_dual_agent_workflow_allowed_policy_runs_producer_without_blocking_on_missing_receipts`. It asserts the supervisor attempts producer execution and records missing receipts as a blocked production event while P13 accepts the allowed policy and synthesis continues.

...
- AutoResearch policy evolution accepts only `.supervisor/policy-overlay.yaml` targets, including rollback pointers.
- Quality trend metrics persist and query `policy_overlay_hash` and `policy_proposal_id`.
- Workflow result finalization invokes regression verification after trend recording and drafts rollback proposals only.
...
6. Wire post-trend regression verification into workflow result finalization so a degraded live overlay proposal drafts a rollback without applying it.
7. Preserve two human touchpoints: operator activation for experiments and operator approval for policy apply/rollback.

## Validation

- `tests/test_policy_overlay.py` proves overlay injection, snapshot hashing, and workflow-result rollback drafting.
- `tests/test_autoresearch_policy_evolution.py` proves policy proposals and rollback pointers target only the overlay and do not auto-apply.
...
- Rollback pointers could be crafted independently of proposal creation, so rollback execution must re-check the overlay target boundary.

## Traceability

- P1 is covered by `test_applied_overlay_changes_next_gate_instruction_and_records_hash`.
- P2 is covered by `test_policy_evolution_rejects_non_overlay_apply_target` and `test_policy_rollback_rejects_non_overlay_target_pointer`.
- P3 is covered by `test_quality_trends_rows_include_policy_overlay_hash_and_proposal_id`.
- P4 is covered by `test_policy_regression_drafts_one_rollback_and_does_not_apply` and `test_workflow_result_drafts_policy_regression_rollback_from_recorded_trends`.
...
- P5 recurrence into AutoResearch is covered by `test_autoresearch_signal_generator_reads_reviewer_probe_and_lesson_signals`.

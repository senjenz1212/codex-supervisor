# Agentic Lead Executor Wiring Implementation Plan
...
1. Add RED tests for required-policy producer invocation, missing-receipt blocking, roster validation, solo-exception gate scoping, non-supervisor evidence downgrade, `docs/dual-agent/` provenance over-crediting, and timeout cleanup. Add CLI payload round-trip as regression/characterization coverage.
